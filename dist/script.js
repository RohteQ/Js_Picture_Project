...
const burger = (menuSelector, burgerSelector) => {
  ...
  burgerElem.addEventListener('click', () => {
    //while width < 993 px => showing menu
    ...
    elem.addEventListener(event, e => {
      ...
      function calcFunc(state) {
        ...
        if (sizeBlock.value == '' || materialBlock.value == '') {
          ...
          if (sizeBlock.value && materialBlock.value || sizeBlock.value && materialBlock.value && promocodeBlock) resultBlock.textContent = sum;
        }
...
/***/ "./src/js/modules/clearObjects.js":
/*!****************************************!*\
  !*** ./src/js/modules/clearObjects.js ***!
  \****************************************/
/***/ (function(__unused_webpack_module, __webpack_exports__, __webpack_require__) {
...
__webpack_require__.r(__webpack_exports__);
const drop = () => {
  const fileInputs = document.querySelectorAll('[name="upload"]'); //massive of events
  ...
  fileInputs.forEach(input => {
    input.addEventListener('drop', e => {
      ...
      if (input.closest('main')) {
        ...
        postData('assets/server.php', formData).then(res => console.log(res)).catch(() => console.log('Ошибка'));
      }
...
const filter = () => {
  const menu = document.querySelector('.portfolio-menu'),
        ...
        no = document.querySelector('.portfolio-no'); //markType= collection of elements
...
/* harmony import */ var _clearObjects__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./clearObjects */ "./src/js/modules/clearObjects.js");
    ...
    form.addEventListener('submit', e => {
      ...
      (0,_services_requests__WEBPACK_IMPORTED_MODULE_1__.postData)(api, formData).then(res => {
        ...
        setTimeout(() => {
          ...
          clearInputs();
          (0,_clearObjects__WEBPACK_IMPORTED_MODULE_2__["default"])(state);
...
const mask = selector => {
  ...
  function createMask(event) {
    ...
    def = matrix.replace(/\D/g, ''),
        //dynamic based
    val = this.value.replace(/\D/g, ''); //when user put smth in matrix if he remove 7 and +,we don't allow it
...
const modals = state => {
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector, destroy = false) {
    ...
    trigger.forEach(item => {
      let event = item.addEventListener('click', e => {
        ...
        btnPressed = true; //destroing gift img after click
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      //paragraph 14 
      let display; //if every modal which enumiration right now showing to the user
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', () => {
      ...
      let scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); //2d condition = whether the user scrolled to the end of the page
  ...
  links.forEach(link => {
    link.addEventListener('click', function (e) {
      ...
      function frame(time) {
        ...
        document.documentElement.scrollTo(0, r); //when stop animation
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    (0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.getResource)('assets/db.json') //massive to func createCards
    .then(res => createCards(res.styles)).catch(() => {
      status();
    });
    this.remove();
  });
...
const sliders = (slides, dir, prev, next) => {
  ...
  activateAnimation(); //using paused we stopped animation of definite slider
...
/* harmony import */ var _modules_clearObjects__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ./modules/clearObjects */ "./src/js/modules/clearObjects.js");
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"])(modalState, '.calc-price');
